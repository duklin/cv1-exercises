...
class KalmanFilter(object):
    ...
    def track(self, xt):
        ...
        gain = state_cov_pred @ self.phi.T @ np.linalg.inv(
            self.sigma_m + self.phi @ state_cov_pred @ self.phi.T)

        self.state = state_pred + gain @ (xt - self.phi @ state_pred)
        self.covariance = (
            np.eye(self.state.shape[0]) - gain @ self.phi) @ state_cov_pred
