...
def load_data(fpath, radius):
    ...
    def u(i): return radius * np.cos(i) + w / 2
    def v(i): return radius * np.sin(i) + h / 2
    V = np.array(
        [(u(i), v(i)) for i in np.linspace(0, 2 * np.pi, n + 1)][:-1],
        'int32')
    return Im, V
...
def compute_gradients(img, window_size: int = 3):
    ...
    img_blur = cv2.GaussianBlur(
        img, (0, 0), 5, borderType=cv2.BORDER_DEFAULT)
    grad_x = cv2.Sobel(img_blur, -1, dx=1, dy=0, ksize=window_size)
    grad_y = cv2.Sobel(img_blur, -1, dx=0, dy=1, ksize=window_size)

    grad_magnitude_2 = np.square(grad_x.astype(
        np.int32)) + np.square(grad_y.astype(np.int32))
...
def neighbor_coordinates(center_loc: tuple, window_size: int, img_size: tuple):
    if window_size % 2 == 1:
        x, y = np.meshgrid(range(max(0, center_loc[1] - window_size // 2), min(img_size[0], center_loc[1] + window_size // 2 + 1)),
                           range(max(0, center_loc[0] - window_size // 2), min(img_size[1], center_loc[0] + window_size // 2 + 1)))
    else:
...
def viterbi_dp(states, unary_costs, avg_dist):
    ...
    indices = np.zeros(
        (unary_costs.shape[0], unary_costs.shape[1] - 1), np.int8)
    ...
    for i in range(states.shape[1] - 1):
        for j in range(states.shape[0]):
            ...
            total_cost = alpha * binary_cost + beta * \
                accumulated_cost[:, i] + beta * accumulated_cost[j, i + 1]
...
def run(fpath, radius):
    ...
    for t in range(n_steps):
        states = np.array([neighbor_coordinates(V[i], window_size, Im.shape)
                          for i in range(len(V))]).transpose(2, 0, 1)
